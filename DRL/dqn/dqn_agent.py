...
class ReplayMemory:
    ...
    def sample(self):
        ...
        batch_s_tensor = torch.as_tensor(np.asarray(batch_s_), dtype=torch.float32)
        
        return batch_s_tensor, batch_a_tensor, batch_r_tensor, batch_done_tensor, batch_s_tensor
...
class DQNAgent:
    def __init__(self, n_input, n_output):
        ...
        self.online_net = DQN(n_input=self.n_input, n_output=self.n_output)
        self.target_net = DQN(n_input=self.n_input, n_output=self.n_output)
